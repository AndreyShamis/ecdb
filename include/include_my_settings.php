...
class My {
	public function Settings() {
		...
		if(isset($_POST['submit'])) {
			...
			$GetDataComponent = $db->query("SELECT passwd FROM members WHERE member_id = ".$owner."");
			$executesql = $db->fetch_assoc($GetDataComponent);
			
			$firstname			=	strip_tags($db->real_escape_string($_POST['firstname']));
			$lastname			=	strip_tags($db->real_escape_string($_POST['lastname']));
			
			$mail				=	strip_tags($db->real_escape_string($_POST['mail']));
			$oldpass			=	strip_tags($db->real_escape_string($_POST['oldpass']));
			$newpass			=	strip_tags($db->real_escape_string($_POST['newpass']));
			
			$measurement		=	strip_tags($db->real_escape_string($_POST['measurement']));
			$currency			=	strip_tags($db->real_escape_string($_POST['currency']));
