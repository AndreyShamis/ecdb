...
class ProjectShow {	
	public function ProjectShowComponents() {
		...
		while($showDetails = $db->fetch_assoc($sql_exec)) {
			...
			$ShowQuant = $db->query("SELECT projects_data_quantity FROM projects_data WHERE projects_data_component_id = '$comp_id' AND projects_data_project_id = '$project_id'");
			$quant = $db->fetch_assoc($ShowQuant);
