	...
	public function Search() {
		...
		if(isset($_REQUEST['q'])) {
			...
			if ($find == '') {
				...
				$sql_exec = $db->query($SearchQuery);
				$anymatches = $db->count($sql_exec);
				if ($anymatches == 0) {
					echo '<div class="message red">';
...
//						echo '<span class="imgB"><img src="'. $image. '" /></span></a></td>';
	...
	public function Add() {
		...
		if(isset($_POST['submit']) or isset($_POST['update'])) {
			...
			$comment			=	strip_tags($db->real_escape_string($_POST['comment']));
			$order_quantity		=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['orderquant'])));
			$project_quantity	=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['projquant'])));
			$price				=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['price'])));
			$location			=	strip_tags($db->real_escape_string($_POST['location']));
			$manufacturer		=	strip_tags($db->real_escape_string($_POST['manufacturer']));
			$package			=	strip_tags($db->real_escape_string($_POST['package']));
			$pins				=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['pins'])));
			$scrap				=	strip_tags($db->real_escape_string($_POST['scrap']));
			$smd				=	strip_tags($db->real_escape_string($_POST['smd']));
			$public				=	strip_tags($db->real_escape_string($_POST['public']));
			$width				=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['width'])));
			$height				=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['height'])));
			$depth				=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['depth'])));
			$weight				=	str_replace(',', '.', strip_tags($db->real_escape_string($_POST['weight'])));
			$datasheet			=	strip_tags($db->real_escape_string($_POST['datasheet']));
			$url1				=	strip_tags($db->real_escape_string($_POST['url1']));
			$url2				=	strip_tags($db->real_escape_string($_POST['url2']));
			$url3				=	strip_tags($db->real_escape_string($_POST['url3']));
			$url4				=	strip_tags($db->real_escape_string($_POST['url4']));
			...
			if ($name == '') {
				...
				if(isset($_POST['submit'])) {
					...
					$sql_exec = $db->query($sql) ;///or die(mysql_error());
					...
					echo $component_id;
					echo '">View component (';
						$result = $db->query("SELECT name FROM data WHERE id = '$component_id'");
				...
				if(isset($_POST['update'])) {
					...
					if (!empty($project) && !empty($project_quantity)) {
						...
						$sql_exec = $db->query($proj_add);// or die(mysql_error());
						echo $project;
						echo ' Owner ';
						echo $owner;
						echo ' id ';
